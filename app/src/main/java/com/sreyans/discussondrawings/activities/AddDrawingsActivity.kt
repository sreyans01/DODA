                ...
                viewModel.getDrawingImageUrl(uri)
                    .observeOnce(this@AddDrawingsActivity, Observer { msg ->
                        when (msg.status) {
                            ...
                            Status.ERROR -> {
                                Toast.makeText(this@AddDrawingsActivity, msg.message, Toast.LENGTH_LONG)
                            }
                            else -> {
        ...
        binding.uploadDrawingBtn.setOnClickListener {
            if (!TextUtils.isEmpty(binding.title.text)) {
                if (binding.addDrawingLayoutParent.visibility == View.GONE) {
                    ...
                    Log.i("KKKKKKKK", "${markers.size} jjjj marker pop")
                    ...
                    Toast.makeText(this@AddDrawingsActivity, getString(R.string.select_drawing), Toast.LENGTH_LONG)
                }
            } else {
                Toast.makeText(this@AddDrawingsActivity, getString(R.string.title_cannot_be_empty), Toast.LENGTH_LONG)
            }
        }
                ...
                when (event?.action) {
                    MotionEvent.ACTION_DOWN -> {
                        if(firstTouch && (System.currentTimeMillis() - time) <= 300) {
                            ...
                            image.setImageDrawable(ContextCompat.getDrawable(this@AddDrawingsActivity, R.drawable.ic_marker_red))
